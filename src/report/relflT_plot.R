...
# Re ~ T per placette
plt_mdl_p7 <- function(df, Tsel, xlab="photosynthèse brute", param=mlabel){
  #   set up data frames
  ...
  plt <- ggplot(df, aes(x=T_value, y=flux, colour = ctrl))+
    ...
    geom_text(data=filter(param, equation == "exponential"), aes(label=paste("R^2==", R2, "(exp)~~~Q10==",Q10 ,sep="")), x=txt_x, y=(txt_y-4), hjust=0, size=4, parse=T, color = "blue")+
