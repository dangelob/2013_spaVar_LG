...
## ctrl fact
df_p <- svCtrlFact %>%
  select(ID_camp, placette, date, NEE_time, RH_m)%>%
  do(na.omit(.))%>%
  mutate(p_ts=as.POSIXct(paste(date, NEE_time, sep=" "), format="%Y-%m-%d %H:%M"))


df_pm <- df_p %>%
  select(ID_camp, date, NEE_time, RH_m)%>%
  mutate(date = paste(as.character(date), NEE_time))%>%
  mutate(p_ts = as.POSIXct(date, format="%Y-%m-%d %H:%M"))%>%
  group_by(ID_camp)%>%
  summarise(p_ts = min(p_ts, na.rm=T), RH_m = mean(RH_m, na.rm=T))%>%
  select(-ID_camp)

df_pm <- as.data.frame(df_pm)

#add line to force Herbacée at 0 at the start (during winter)
# newrow <- data.frame(ID_camp=0, p_ts=c(as.POSIXct("2013-01-01 00:00:00"),     as.POSIXct("2013-12-01 00:00:00")), A=NA, H=0, M=NA, IVcov=c(0.15,0.2))
# 
...
xdf_pm <- xts(df_pm$RH_m, order.by = df_pm$p_ts)
...
# xts
df1 <- merge(xdf_c, xdf_pm)
# interpol
df1$xdf_pmc <- na.approx(df1$xdf_pm)
...
cRH <- tt %>%
  select(timestamp, xdf_pmc)%>%
  rename(RH = xdf_pmc)
  

write.csv(cRH, "../../../data/processed/pdata_to_cdata/cRH_all_hour.csv", row.names=FALSE, quote=FALSE)
```
