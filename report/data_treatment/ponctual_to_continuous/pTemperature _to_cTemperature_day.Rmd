...
```{r setup}
# Attention fichier en travaux (arrêt du dev car inutile a priori, les moyennes journalière pour les interpolations sont faites directement a partir des données de la station météo.)
...
df_p <- left_join(svCtrlFact, svTemperature) %>%
  select(date, TairDeb, TairFin, Tsurf, T5, T10, T15, T20, T25, T20, T30, T40, T50, T60, T70, T80, T90, T100, Tair)%>%
  group_by(date)%>%
