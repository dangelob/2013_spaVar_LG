...
df_pm <- df_p %>%
  group_by(ID_camp)%>%
  summarise(p_ts=mean(p_ts, na.rm=T), A=mean(A, na.rm=T), H=mean(H,na.rm=T), M=mean(M,na.rm=T), IVcov=mean(IVcov,na.rm=T))%>%
  ungroup()
...
write.csv(cVegm, "../../../data/processed/pdata_to_cdata/cVeg_all.csv", row.names=FALSE, quote=FALSE)
```
