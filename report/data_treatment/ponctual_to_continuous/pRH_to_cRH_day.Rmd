...
df_pm <- df_p %>%
  select(ID_camp, date, RH_m)%>%
  group_by(ID_camp)%>%
  summarise(date = min(date, na.rm=T), RH_m = mean(RH_m, na.rm=T))%>%
...
write.csv(cRH, "../../../data/processed/pdata_to_cdata/cRH_all_day.csv", row.names=FALSE, quote=FALSE)
```
