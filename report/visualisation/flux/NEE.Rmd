...
# File name : /2013_spavar_LG/report/visualisation/flux/NEE.Rmd
...
ggplot(mdf, aes(x=date, y=netCO2F))+
  geom_point(size=3)+
  annotate("text", x=as.Date("2015-02-01"), y=10, label="N=20", size=4)+
  annotate("text", x=as.Date("2013-06-15"), y=10, label="2013", size=5)+
  annotate("text", x=as.Date("2014-06-15"), y=10, label="2014", size=5)+
  annotate("text", x=as.Date("2013-12-01"), y=3.85, label="2,85", size=5, color=orange)+
  annotate("segment", x=as.Date("2013-03-05"), xend=as.Date("2013-12-15"), y=2.85, yend=2.85, linetype="dashed", size=1, color=orange)+
  annotate("text", x=as.Date("2014-12-01"), y=3.93, label="2,93", size=5, color=orange)+
  annotate("segment", x=as.Date("2014-03-15"), xend=as.Date("2014-12-15"), y=2.93, yend=2.93, linetype="dashed", size=1, color=orange)+
  geom_linerange(aes(x=date, ymin=netCO2F-netCO2F_sd, ymax=netCO2F+netCO2F_sd))+
  labs(y=lab_Y, x="")+
  scale_x_date(breaks = date_breaks("2 months"), labels = date_format("%b"))+
  theme_classic()+
  theme(plot.margin=unit(c(0,0,0,0),"mm"))
ggsave("VS_NEE_avg.pdf", path=savpth, width=7, height=3)
```
...
tbl <- as.data.frame(mdf[,1:4])%>%
  mutate(NEE=round(netCO2F, 2), NEE_sd=round(netCO2F_sd,2))%>%
  select(ID_camp, date, NEE, NEE_sd)

knitr::kable(tbl, caption="NEE fluxes averaged by campaign")
```
