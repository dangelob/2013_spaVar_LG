...
ggplot(dfm, aes(x=date, y=pH, group=year))+
  geom_point()+
  geom_line()+
  geom_line(aes(x=date, y=pH+pH_sd), linetype="dashed")+
  geom_line(aes(x=date, y=pH-pH_sd), linetype="dashed")+
#   geom_line(aes(group=profondeur))+
#   facet_wrap(~year)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=30, hjust=1))
ggsave("pH_mean_evolution.pdf", path=savpth)
```
...
```{r, echo=FALSE, warning=FALSE}
ggplot(dfm, aes(x=date, y=cond, group=year))+
  geom_point()+
  geom_line()+
  geom_line(aes(x=date, y=cond+cond_sd), linetype="dashed")+
  geom_line(aes(x=date, y=cond-cond_sd), linetype="dashed")+
#   geom_line(aes(group=profondeur))+
#   facet_wrap(~year)+
  theme_bw()+
  theme(axis.text.x=element_text(angle=30, hjust=1))
ggsave("cond_mean_evolution.pdf", path=savpth)
```
