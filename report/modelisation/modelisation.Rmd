...
#   source("/home/dangelo/Documents/4.ScienceStuff/2.Projects/2013_JourNuit/src/3_functions/reportFn.R")
  ...
  output_pth <- "/home/dangelo/Documents/4.ScienceStuff/2.Projects/2014_CrossModelisation/graphs"
#   cat("loadenv : done \n")
  library(zoo)
  }
...
```{r Re_T15_dat, fig.height=5.5, fig.width=5.5, tidy=TRUE, echo=FALSE}
mdf <- df[,c("placette","ReAbs","T15","WT","ID_camp", "date")]
colnames(mdf)[3] <- "Tmprt"
mdf <- mdf[which(!is.na(mdf$ReAbs) & !is.na(mdf$Tmprt)),]
...
```{r Re_T15_graph, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, w=12, h=8, dpi=100}
ggplot(result, aes(x=Tmprt, y=ReAbs))+
#   ggtitle(lab_ReTps)+
  geom_point()+
  geom_line(data=m_lines, aes(x=Tmprt, y=mdl1), color = "red")+
...
ggplot(result, aes(x=pmdl3, y=ReAbs))+
  ...
  annotate("text",x=1.5, y=19, label=paste0("R² = ",coef$mdl3_sqR))+
  guides(color=FALSE)+
  coord_fixed(ratio = 1)+#, ylim=c(0,35), xlim=c(0,35))+
