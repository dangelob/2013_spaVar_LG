---
title: "NEE relations with environmental variables"
...
```{r, message=FALSE}
# File name : /2013_spavar_LG/report/rel_flux_envir/cor_NEE.Rmd
...
```{r, fig.height=8, fig.width=8}
sel <- dfgesm %>%
  select(NEE,Tair, T5, T10, T20, T30, T40, T50, T60, T70, T80)
pairs(sel, lower.panel = panel_cor, cex.labels = 2)
```

## NEE vs temperature: R2 evolution with depth

Using NEE=a*exp(b*Temperature)

```{r, fig.width=5, fig.height=7}
t <- dfgesm %>%
  select(NEE, WTL, A, H, M, IVcov, Tair, T5, T10, T20, T30, T40, T50, T60, T70, T80, T90)

# plot(t$NEE~t$T50)
# summary(lm(t$NEE~t$T40))
R1 <- summary(lm(log(t$NEE)~t$Tair))$r.squared
R2 <- summary(lm(log(t$NEE)~t$T5))$r.squared
R3 <- summary(lm(log(t$NEE)~t$T10))$r.squared
R4 <- summary(lm(log(t$NEE)~t$T20))$r.squared
R5 <- summary(lm(log(t$NEE)~t$T30))$r.squared
R6 <- summary(lm(log(t$NEE)~t$T40))$r.squared
R7 <- summary(lm(log(t$NEE)~t$T50))$r.squared
R8 <- summary(lm(log(t$NEE)~t$T60))$r.squared
R9 <- summary(lm(log(t$NEE)~t$T70))$r.squared
R10 <- summary(lm(log(t$NEE)~t$T80))$r.squared
R11 <- summary(lm(log(t$NEE)~t$T90))$r.squared
# summary(lm(log(t$NEE)~t$T100))$r.squared
...
ggsave("NEE_T_exp.pdf", path=savpth, width=3.5, height=5)
```

## NEE vs vegetation and water table level


```{r}
lin_A <- round(summary(lm(t$NEE~t$A))$r.squared,2)
lin_H <- round(summary(lm(t$NEE~t$H))$r.squared,2)
lin_M <- round(summary(lm(t$NEE~t$M))$r.squared,2)
lin_IVcov <- round(summary(lm(t$NEE~t$IVcov))$r.squared,2)
lin_WTL <- round(summary(lm(t$NEE~t$WTL))$r.squared,2)

exp_A <- round(summary(lm(log(t$NEE)~t$A))$r.squared,2)
exp_H <- round(summary(lm(log(t$NEE)~t$H))$r.squared,2)
exp_M <- round(summary(lm(log(t$NEE)~t$M))$r.squared,2)
exp_IVcov <- round(summary(lm(log(t$NEE)~t$IVcov))$r.squared,2)
exp_WTL <- round(summary(lm(log(t$NEE)~t$WTL))$r.squared,2)
...
knitr::kable(linexp, caption="NEE relation with environmental variable using linear and exponential equations (R2 values)")
